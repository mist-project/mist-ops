...
export TF_VAR_MIST_DOCKER_DEV_USERNAME=$(op read op://mistiop/mist-docker-dev/username)
export TF_VAR_MIST_DOCKER_DEV_SSH_PUBKEY=$(op read op://mistiop/mist-docker-dev/public\ key)

# Check if secret fetching was successful before proceeding
if [ -z "$TF_VAR_MIST_DOCKER_DEV_USERNAME" ] || \
   [ -z "$TF_VAR_MIST_DOCKER_DEV_SSH_PUBKEY" ]; then
    echo "Error: One or more secrets could not be fetched from 1Password."
    exit 1
fi

# Only provisions the Docker development VM
terraform apply --auto-approve --target=proxmox_vm_qemu.mist-docker-dev

# Unset the exported variables after terraform apply completes
unset TF_VAR_MIST_DOCKER_DEV_USERNAME
unset TF_VAR_MIST_DOCKER_DEV_SSH_PUBKEY
